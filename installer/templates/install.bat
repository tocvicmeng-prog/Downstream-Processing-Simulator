...
echo          DPSim requires Python ^>=3.11 and ^<3.13 (ADR-001).
...
set "WHEEL="
for %%W in (wheels\downstream_processing_simulator-*-py3-none-any.whl) do set "WHEEL=%%W"
if not defined WHEEL goto err_wheel_missing
if not exist "%WHEEL%" goto err_wheel_missing
echo [install] Found wheel: %WHEEL%

echo [install] Step 6: installing DPSim wheel with extras %EXTRAS%.
...
"%VPY%" -c "import dpsim; print('  dpsim', dpsim.__version__, 'imported OK')"
...
"%VPY%" -m dpsim run configs\fast_smoke.toml --quiet
...
echo           Fix: uninstall, then re-install. DPSim per-user install goes
echo           into %%LOCALAPPDATA%%\Programs\DPSim which needs no admin.
...
:err_wheel_missing
echo.
echo [install] ERROR: no DPSim wheel found.
echo           Expected: wheels\downstream_processing_simulator-^<version^>-py3-none-any.whl
echo           The installer may have been truncated or extracted incorrectly.
echo           Re-download the Setup.exe from
echo           https://github.com/tocvicmeng-prog/Downstream-Processing-Simulator/releases/latest
if "%NONINTERACTIVE%"=="" pause
endlocal
exit /b 5
...
:err_pip_install
echo.
echo [install] ERROR: pip install of the DPSim wheel failed.
...
:err_import
echo.
echo [install] ERROR: cannot import dpsim after install.
