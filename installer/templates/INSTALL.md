...
If you skip the PATH step the DPSim installer will not find Python
...
3. Install the DPSim wheel with the **UI** and **optimisation** extras
...
- `.venv\` — the virtual environment. Everything DPSim needs lives
...
DPSim virtual environment activated. Try:

```
dpsim run configs\default.toml
dpsim sweep --rpm-min 3000 --rpm-max 15000 --rpm-steps 5
dpsim uncertainty --n-samples 50
dpsim info
```

`dpsim --help` lists every subcommand.
...
1. On an internet-connected machine with matching Python and
   ...
   ```
   pip download dpsim[ui,optimization] -d wheels_cache
   ```
