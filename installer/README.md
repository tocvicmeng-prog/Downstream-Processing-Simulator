# DPSim Windows Installer Sources

This directory holds the Inno Setup build assets that produce
`DPSim-<version>-Setup.exe` — the one-click Windows 11 x64
...
Output: `release\DPSim-<version>-Setup.exe` (≈ 2.5 MB).
...
3. `gh release create vX.Y.Z release/DPSim-X.Y.Z-Setup.exe
    release/DPSim-X.Y.Z-Windows-x64.zip --title "…"
