...
| File | Change | LOC |
|---|---|---|
| `src/dpsim/optimization/engine.py` | `robust_cvar_alpha` ctor param; CVaR branch in `_evaluate` | ~45 |
| `src/dpsim/__main__.py` | `--robust-cvar-alpha` CLI flag + routing | ~15 |
...
CLI:
```
python -m dpsim design --robust-cvar-alpha 0.3 --robust-n-samples 10 [...]
```
