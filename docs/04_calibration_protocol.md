...
This protocol defines five wet-lab calibration studies that supply the material-specific constants required by the emulsification simulation system. Each study targets one or more parameters that cannot be obtained from the literature with sufficient accuracy for the specific material grades and conditions used in this project (4.2% w/v agarose + 1.8% w/v chitosan dispersed in Span-80/paraffin oil at 90 degC). The calibrated values feed directly into the `MaterialProperties` dataclass (`src/dpsim/datatypes.py`) and the TOML property database (`data/properties.toml`).
...
**Note:** Intrinsic viscosity of polyelectrolytes depends strongly on ionic strength. Use 0.2 M NaCl + 1% acetic acid as the solvent to screen electrostatic interactions and obtain a reproducible [eta]. This is standard practice for chitosan characterization (Rinaudo et al. 1993).
...
Calibrate the **Alopaeus viscous breakage constant C3** in the breakage rate kernel used by the Level 1 PBE solver (`src/dpsim/level1_emulsification/kernels.py`, function `breakage_rate_alopaeus`). C3 controls how strongly the dispersed-phase viscosity resists droplet breakup:
...
from dpsim.level1_emulsification.solver import solve_pbe
...
Calibrate the **IPN coupling coefficient eta\_coupling** in the double-network modulus model used in `src/dpsim/level4_mechanical/solver.py` (function `double_network_modulus`):
...
**Interfacial tension model** (`src/dpsim/properties/interfacial.py`):
...
**Chitosan intrinsic viscosity** (`src/dpsim/properties/viscosity.py`):
...
Update `src/dpsim/datatypes.py`, class `MaterialProperties`:
...
The pore structure coefficients (A, alpha, beta from Study 4) are currently embedded in the Level 2 gelation analysis. After calibration, update the relevant pore diameter prediction function. If no explicit function exists in the current codebase, add one to `src/dpsim/level2_gelation/pore_analysis.py`:
...
For each calibrated parameter, record the 95% confidence interval. These uncertainties should be entered into the `uncertainty` field of the `PropertyValue` entries in `properties.toml` and used in the uncertainty analysis module (`src/dpsim/uncertainty.py`) for Monte Carlo propagation through the simulation.
