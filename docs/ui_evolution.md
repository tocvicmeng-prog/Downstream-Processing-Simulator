...
> `src/dpsim/visualization/`. Design system direction is in `DESIGN.md`.

## Historical state before P6 lifecycle UI

The v9.0 "Family-First" redesign was the pre-P6 UI architecture. The current
user-facing workflow is lifecycle-first: Target Product Profile, M1
Fabrication, M2 Chemistry, M3 Column Method, Run, Validation & Evidence, and
Calibration.
...
The Streamlit app reloads `dpsim.datatypes` on every rerun via
...
- Current UI code: `src/dpsim/visualization/` (panels, tabs/m1,
  tabs/m2, tabs/m3)
