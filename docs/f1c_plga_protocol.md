...
Add a PLGA / solvent-evaporation platform to DPSim as the third
...
|---|---|---|
| `src/dpsim/level2_gelation/solvent_evaporation.py` | 1D spherical Fickian DCM-depletion solver | ~320 |
| `src/dpsim/level4_mechanical/plga.py` | Gibson-Ashby glassy-porous modulus | ~130 |
| `src/dpsim/properties/plga_defaults.py` | Presets for PLGA 50:50 / 75:25 / 85:15 / PLA | ~160 |
...
| File | Change | Est. LOC |
|---|---|---|
| `src/dpsim/datatypes.py` | Add PLGA fields to MaterialProperties; ensure PolymerFamily.PLGA exists (already shipped) | ~12 |
| `src/dpsim/pipeline/orchestrator.py` | Add `_run_plga` branch in `run_single` | ~110 |
| `src/dpsim/__main__.py` | `run --polymer-family plga --plga-grade 50_50` | ~25 |
| `src/dpsim/config.py` | Recognise `[formulation].plga_grade = "50_50"` via FormulationParameters field | ~3 |
