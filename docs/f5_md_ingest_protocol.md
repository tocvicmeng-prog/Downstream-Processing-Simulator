...
inputs to the DPSim property pipeline. Users running MARTINI 3
...
MD-derived quantities and maps them onto existing DPSim
...
- Running MARTINI or any MD package from within DPSim.
- Pair-potential tabulated-force ingestion (beyond recording the
  values; there is no downstream consumer yet).
- Automatic MARTINI ↔ DPSim bead-type mapping (user provides the
...
| File | Purpose | LOC |
|---|---|---|
| `src/dpsim/md_ingest.py` | `MartiniRecord` dataclass + `load_martini_record(path)` + `apply_chi_to_props(props, record)` | ~220 |
...
- Automatic bead-type mapping from MARTINI → DPSim polymer family.
- CalibrationStore integration so MD-derived χ propagates through
  the existing posterior-sampling infrastructure.
- Reverse direction: emit a MARTINI-shaped record FROM an DPSim
