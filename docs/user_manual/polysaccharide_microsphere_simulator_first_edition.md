# Downstream Processing Simulator - First Edition

**Edition:** 1.1 unified lifecycle documentation set
**Date:** 2026-04-25
**Audience:** Downstream-processing technicians, junior R&D researchers,
first-time users of the DPSim pipeline.
...
The simulator's purpose is simple: **turn a written lifecycle recipe
into predicted microsphere-media behavior before you touch the bench.**
A recipe describes the target product profile, M1 double-emulsification
fabrication, M2 functionalization chemistry, and M3 affinity-column
method. A product is described by measurable outputs: bead size
distribution, residual oil/surfactant, pore structure, modulus, ligand
density, breakthrough behavior, pressure drop, and evidence level. The
simulator tells you what to expect, with explicit uncertainty,
assumptions, validation gates, and wet-lab caveats.

This manual is organised in three concentric layers. **Part I** teaches
a first-time user to launch the UI, define a lifecycle `ProcessRecipe`,
run the M1/M2/M3 simulation, and interpret validation and evidence.
**Part II** is a reference for choosing M1 polymer platforms and M2
chemistries. **Part III** is for users who want to understand the
science, audit assumptions, or troubleshoot an unexpected result.
...
The DPSim Downstream Processing Simulator (below: "DPSim") is a
multi-scale lifecycle simulator for affinity microsphere media. The
current clean path is driven by a first-class `ProcessRecipe` object
from `dpsim.core.process_recipe` and serialized through
`dpsim.core.recipe_io`. The same recipe is consumed by the UI, CLI,
validation layer, SOP export, and lifecycle orchestrator.

DPSim has three process modules:

| Module | Wet-lab stage | Main outputs |
|---|---|---|
| M1 | Double-emulsification microsphere fabrication | bead DSD, d32/d50, pore and modulus estimates, wash residuals |
| M2 | Functionalization, reinforcement, ligand coupling | site balance, ligand density, activity retention, leaching/wash caveats |
| M3 | Affinity-column operation | pack/equilibrate/load/wash/elute method behavior, breakthrough, pressure, DBC |

The inherited M1 numerical kernels still report four internal model
levels. These are implementation levels, not separate user workflows:
...
L3 because its ionic gelation is the crosslinking. These levels feed
M1 material properties into M2 and M3; users should normally reason in
terms of the M1/M2/M3 lifecycle.
...
- Did M1 washing reduce oil/surfactant carryover below the target
  screening limits?
- Will my functionalization sequence preserve the required reactive-site
  and mass balance?
- What ligand density, activity retention, and leaching risk should I
  expect from M2?
- What will the final bead modulus be?
- For chromatography: what breakthrough, pressure drop, and DBC trend
  should I expect under the defined method?
- Which validation blockers, calibration-domain warnings, evidence tier,
  assumptions, and wet-lab caveats apply?
...
- It does not create a GMP batch record. The exported wet-lab protocol is
  a development SOP scaffold that must be reviewed under local safety and
  quality systems.
- It does not prove a resin is fit for clinical, diagnostic, or
  manufacturing use without calibration, release assays, and process
  validation.
...
|---|---|---|
| `validated_quantitative` | Locally validated against the specific system and response | Suitable for design within the validated domain |
...
| `semi_quantitative` | Literature-parameterised or mechanistically plausible but not locally calibrated | Trends are useful; magnitudes are approximate |
| `qualitative_trend` | Directional or ranking-only | Use only for screening and hypothesis generation |
| `unsupported` | Chemistry, unit, regime, or operation is not represented | Do not use for decisions |

The UI marks reported values with their tier and shows the weakest
evidence level across the lifecycle. M3 is never allowed to claim
stronger evidence than the M2 media contract that supplies the ligand
and site-balance basis.

## 2. Workflow Overview

All lifecycle runs follow the same seven-step workflow.
...
7. Calibration And Wet-Lab Comparison
```

The UI implements this flow directly. Each control writes into the
active `ProcessRecipe`, and each result view reads the same recipe and
lifecycle result. The legacy M1/M2/M3 tabs may still exist for narrow
debugging, but they are not the authoritative user workflow.
...
This starts the Streamlit dashboard at `http://localhost:8501`. If the
browser does not open automatically, paste the URL into a local browser.
The manual and Appendix J protocol PDF are available from the dashboard.

### 3.2 Entering a minimal recipe

Use the workflow pages in order:

1. **Target Product Profile:** define bead d50, pore size, minimum
   modulus, ligand, analyte, maximum pressure drop, and residual
   oil/surfactant screening limits.
2. **M1 Fabrication:** define phase preparation, emulsification,
   cooling/gelation, and wash settings. Archive measured DSD quantiles
   if microscopy or laser diffraction data are available.
3. **M2 Chemistry:** choose a staged functionalization template or edit
   activation, spacer insertion, coupling, blocking/quenching, washing,
   and storage-buffer exchange steps.
4. **M3 Column Method:** define packing, equilibration, load, wash, and
   elution conditions including pH, conductivity, residence time, bed
   height, and flow rate.
5. **Run Simulation:** execute the lifecycle orchestrator.
6. **Validation & Evidence:** inspect blockers, warnings, evidence
   ladder, scientific diagnostics, and wet-lab caveats.
7. **Calibration:** compare simulated outputs with wet-lab data and
   confirm whether the run is inside calibration domains.

First-time users should start from the default Protein A affinity-media
recipe. It is conservative and intended for training, not optimized
manufacture.
...
By default, runtime output is written under the per-user DPSim runtime
directory rather than blocked system temp directories or fragile
OneDrive paths. Override these locations when needed:
...
The lifecycle UI reports:

- M1: bead DSD summary, DSD quantiles, pore/modulus estimates, residual
  oil and residual surfactant after washing.
- M2: functional-site inventory, ligand density, side-reaction and
  hydrolysis caveats, activity-retention estimate, ligand leaching and
  free-protein wash assay requirements.
- M3: pack/equilibrate/load/wash/elute method summary, breakthrough
  curve, pressure profile, operability limits, DBC/recovery trends, and
  Protein A degradation/leaching caveats where applicable.
- Cross-cutting views: validation report, evidence ladder, scientific
  diagnostics, wet-lab SOP draft, calibration status, run history, and
...
3. **Export and run the lifecycle default recipe.** Inspect how the M1
   DSD, M2 Protein A coupling contract, and M3 column method combine into
   one validation/evidence report.
...
## 9. Reading Lifecycle Evidence And Suggestions

After a lifecycle run, the Validation & Evidence view lists blockers,
warnings, scientific diagnostics, and any available suggestions. A
suggestion is advisory unless the underlying model and calibration domain
support the requested decision. For example, a validated pressure-flow
calibration may support a quantitative flow-rate adjustment, while an
uncalibrated pore-size trend should remain qualitative.

### Three canonical sections

Every evidence or suggestion record should be read through the same three
questions:

1. **Model pathway.** Which M1/M2/M3 model produced this statement,
   and which `ProcessRecipe` values were resolved into it?
2. **Target value or range.** Is the output a quantitative value,
   calibrated interval, or direction-only recommendation?
3. **Assumptions and confidence.** Which assumptions, evidence tier,
   calibration-domain checks, and wet-lab caveats apply?

### Qualitative-only behaviour

When the underlying model is only `qualitative_trend`, DPSim must not
present the number as decision-grade. It may show direction-only guidance
and the wet-lab assay needed to upgrade the result. Common examples are
uncalibrated ligand activity retention, resin leaching risk, or M3
breakthrough outside the fitted flow-rate and concentration domain.
...
### Run records are reproducible

The run history, exported recipe, validation report, and protocol draft
are the reproducibility record. If you edit the recipe and re-run, keep
the earlier `ProcessRecipe` and output dossier when comparing wet-lab
results.

### Supported suggestion families
...
Lifecycle-specific checks also report M1 residual carryover, M2 site/mass
balance, M3 pressure/compression limits, Reynolds/Peclet validity, and
calibration-domain violations.

### How this fits with Appendix J

Some suggestions map directly to wet-lab protocols in Appendix J. The
UI shows the recipe, model, evidence, and calibration reason; Appendix J
shows bench execution and safety constraints. Read both before changing
a chemistry or column method.
...
| Output directory | path | — | user runtime output directory | Override with `DPSIM_OUTPUT_DIR` if required |
...
and units are in `src/dpsim/datatypes.py`.
...
Use `python -m dpsim uncertainty config.toml --n-samples 50`.
...
days. DPSim uses a **population balance equation** (PBE) at the
...
Yes. The CLI (`python -m dpsim run ...`) runs without a browser.
...
Use `python -m dpsim sweep` (L1 only) or the `design` subcommand
with a TargetSpec (full pipeline BO). Avoid naive loops over the
UI; the CLI is 10× faster.

**D15. Where do I report bugs?**
GitHub: `tocvicmeng-prog/Downstream-Processing-Simulator`.

## Appendix E. Architectural Ideas & Working Principles

### E.1 The "levels" abstraction

DPSim decomposes microsphere fabrication into four sequential
stages (**L1–L4**) plus lifecycle modules (**M1** fabrication export
contract, **M2** functionalization, **M3** characterization). Each
level has:
...
- A **UI lifecycle view** or diagnostic panel.
...
`dpsim design` takes a **TargetSpec** (desired d32, pore, G_DN,
...
`dpsim.digital_twin.run_replay` is an offline Ensemble Kalman
...
Implementation: `src/dpsim/level1_emulsification/solver.py`.
...
`src/dpsim/digital_twin/enkf.py`.
...
### H.0 Common release gates for M1/M2/M3 bench alignment

Do not treat a simulated recipe as bench-ready until the following
records exist. These are the minimum records required for DPSim results
to be scientifically interpretable.

| Gate | Required record | Why it matters to the simulator |
|---|---|---|
| M1 bead size | Microscopy or laser diffraction DSD with at least d10/d50/d90 and span | DPSim propagates DSD quantiles into M2 accessibility and M3 pressure/breakthrough risk |
| M1 wash residuals | Residual oil and residual surfactant assay or justified proxy | Residuals can suppress coupling, foul columns, and invalidate wet-lab feasibility |
| M1 pore/swelling | Pore imaging, SEC inverse-size calibration, and swelling ratio in column buffer | Pore and swelling control ligand accessibility, mass transfer, and pressure drop |
| M1 mechanics | Single-bead compression or packed-bed pressure-flow curve | M3 operability depends on modulus, bed compression, and pressure limit |
| M2 chemistry | Ligand density, residual active sites, free ligand/protein in wash fractions | Site and mass balance define the media contract passed to M3 |
| M2 stability | Ligand leaching and activity-retention assay in storage/use buffer | M3 evidence cannot exceed the quality of the M2 media contract |
| M3 column | Packing observation, pressure-flow curve, breakthrough curve, recovery, and leaching | Required for calibration and for pressure/compression-domain checks |

All pH values are measured with a calibrated pH meter at the working
temperature or temperature-corrected according to the meter SOP. All
temperature-sensitive protein steps must use pre-chilled buffers and
document the actual slurry temperature, not only the room or bath setpoint.
...
5. Separate the beads by centrifugation or sieving; wash by repeated
   drain/resuspend cycles using the recipe wash-volume ratio and mixing
   efficiency. Warm (40 °C) buffer may improve oil removal, but record
   the actual wash temperature and avoid conditions that swell or
   mechanically damage the beads.
6. Resuspend in a 2 mM genipin solution in buffer at 37 °C;
   incubate 24 h with gentle stirring.
7. Wash thoroughly until the supernatant is visually clear and residual
   oil/surfactant assays meet the target screening limits. Store in
   validated buffer at 4 °C.

QC before M2: record d10/d50/d90, pore-size estimate, swelling ratio,
bulk or single-bead modulus, residual oil, and residual Span-80-equivalent
surfactant. These values should be entered as calibration or assay
records before making quantitative M2/M3 claims.
...
4. Wash beads in PBS or the validated storage buffer until free ligand
   is below the assay threshold. Quench remaining NHS ester with
   ethanolamine or Tris only after the intended ligand-coupling step is
   complete.
5. Store at 4 °C in PBS + 0.05 % sodium azide where azide is permitted.
   Do not use azide with metal-contact plumbing, live-cell workflows, or
   assays where azide interferes.
...
Safety: DCM is volatile, harmful by inhalation and skin contact, a CNS
depressant, and a suspected carcinogen. Perform all DCM handling,
emulsification transfers, evaporation, and waste opening in a certified
fume hood. Use compatible gloves per the SDS, keep containers closed,
and collect all DCM-containing liquids, wipes, and contaminated
consumables as halogenated hazardous waste. Do not rely on odor as a
clearance test; use the laboratory solvent-residual assay or validated
evaporation endpoint before downstream use.
...
| Calibration not applied | AssayRecord fit JSON missing / wrong target_module | Re-run `dpsim ingest`; check CalibrationStore loader logs |
...
| Lifecycle step panel blank | Session state corrupted or Streamlit state lost | Refresh the browser; restart `python -m dpsim ui`; re-load or re-export the `ProcessRecipe` |
...
at* `github.com/tocvicmeng-prog/Downstream-Processing-Simulator`.
