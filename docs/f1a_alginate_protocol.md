...
Add an alginate / Ca²⁺ ionic-gelation platform to DPSim as the first
...
functionalization of alginate (current DPSim terminology).
...
|---|---|---|
| `src/dpsim/level2_gelation/ionic_ca.py` | Shrinking-core diffusion-limited Ca²⁺ front solver | ~400 |
| `src/dpsim/level4_mechanical/alginate.py` | Modulus from G-block content | ~150 |
| `src/dpsim/properties/alginate_defaults.py` | Default material properties for alginate (IFT, viscosity curves, f_G, D_Ca, binding stoich) | ~150 |
| `src/dpsim/reagent_library_alginate.py` | Alginate crosslinker/source entries (CaCl₂, internal gelation with GDL + CaCO₃) | ~200 |
...
| File | Change | Est. LOC |
|---|---|---|
| `src/dpsim/datatypes.py` | Add `PolymerFamily` enum (AGAROSE_CHITOSAN | ALGINATE); `MaterialProperties.polymer_family`; alginate-specific fields | ~30 |
| `src/dpsim/pipeline/orchestrator.py` | Dispatch L2 by `polymer_family` | ~40 |
| `src/dpsim/level1_emulsification/solver.py` | No-op for alginate unless viscosity curve differs | ~0 |
| `src/dpsim/config.py` | TOML parser recognises `polymer_family = "alginate"` key | ~20 |
