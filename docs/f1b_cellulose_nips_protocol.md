...
to DPSim as the second non-chitosan-agarose microsphere family.
...
surface functionalization, post-NIPS covalent crosslinking.
...
|---|---|---|
| `src/dpsim/level2_gelation/nips_cellulose.py` | Coupled-diffusion + Cahn-Hilliard NIPS solver | ~550 |
| `src/dpsim/level4_mechanical/cellulose.py` | phi^alpha modulus law | ~140 |
| `src/dpsim/properties/cellulose_defaults.py` | Defaults for 4 solvent systems | ~180 |
| `src/dpsim/reagent_library_cellulose.py` | SolventProfile + NonSolventProfile entries | ~220 |
...
| File | Change | Est. LOC |
|---|---|---|
| `src/dpsim/datatypes.py` | Extend `PolymerFamily` use (enum value already exists); add `MaterialProperties` fields: `cellulose_solvent`, `chi_polymer_solvent`, `chi_polymer_nonsolvent`, `D_solvent`, `D_nonsolvent`, `K_cell`, `alpha_cell`, `M_n_cellulose` | ~40 |
| `src/dpsim/pipeline/orchestrator.py` | Add `PolymerFamily.CELLULOSE` branch in `run_single`: `_run_cellulose(...)` mirrors `_run_alginate` shape | ~110 |
| `src/dpsim/config.py` | Recognise `[formulation].solvent_system = "naoh_urea"` etc., `[formulation].nonsolvent_bath = "water"` | ~30 |
| `src/dpsim/__main__.py` | `run --polymer-family cellulose --solvent-system naoh_urea --nonsolvent water` | ~30 |
