"""CLI entry point for Downstream Processing Simulator."""
...
from .runtime_paths import configure_runtime_environment, default_output_dir
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="dpsim",
        description=(
            "Lifecycle simulation of microsphere fabrication, functional "
            "ligand crosslinking, and affinity chromatography performance"
        ),
    )
    ...
    run_p.add_argument(
        ...
        help="Alginate gelant preset (Phase 2c polish). Applies the "
             "profile's effective Ca2+ concentration to "
    ...
    run_p.add_argument(
        ...
        help="Cellulose solvent-system preset (F1-b Phase 2 / 3). "
             "Patches MaterialProperties with the preset's chi, D, and "
    ...
    run_p.add_argument("--output", "-o", default=str(default_output_dir("m1")), help="Output directory")
    run_p.add_argument("--quiet", "-q", action="store_true")

    # lifecycle command
    life_p = sub.add_parser(
        "lifecycle",
        help="Run clean-slate M1->M2->M3 lifecycle simulation",
    )
    life_p.add_argument("config", nargs="?", default=None)
    life_p.add_argument(
        "--recipe",
        default=None,
        help="Path to ProcessRecipe JSON/TOML. If omitted, the lifecycle CLI bridges the legacy config into a recipe.",
    )
    life_p.add_argument(
        "--export-recipe",
        default=None,
        help="Write the ProcessRecipe used for this lifecycle run to JSON/TOML.",
    )
    life_p.add_argument(
        "--no-dsd",
        action="store_true",
        help="Skip representative DSD propagation screen for faster lifecycle smoke runs.",
    )
    life_p.add_argument(
        "--dsd-mode",
        choices=["representative", "adaptive"],
        default="representative",
        help=(
            "DSD propagation mode. 'representative' uses requested quantiles; "
            "'adaptive' uses non-zero DSD bins up to --dsd-max-representatives "
            "then collapses to bounded-cost volume-probability representatives."
        ),
    )
    life_p.add_argument(
        "--dsd-quantiles",
        default="0.10,0.50,0.90",
        help="Comma-separated quantiles for representative DSD lifecycle mode.",
    )
    life_p.add_argument(
        "--dsd-max-representatives",
        type=int,
        default=9,
        help="Maximum DSD representatives used by adaptive lifecycle propagation.",
    )
    life_p.add_argument(
        "--dsd-breakthrough",
        action="store_true",
        help=(
            "Run M3 LRM breakthrough for every DSD representative. This is "
            "slower than pressure/capacity screening but reports DSD-weighted "
            "DBC tail metrics."
        ),
    )
    life_p.add_argument(
        "--output",
        "-o",
        default=str(default_output_dir("downstream_lifecycle")),
        help="Output directory for lifecycle stage artifacts",
    )
    life_p.add_argument("--quiet", "-q", action="store_true")
    ...
    opt_p.add_argument("--output", "-o", default=str(default_output_dir("optimization")))
    ...
    des_p.add_argument(
        ...
        type=float, default=0.0,
        help="F4-b: CVaR alpha tail-risk aggregation. alpha in (0, 1]. Takes "
    ...
    des_p.add_argument("--output", "-o", default=str(default_output_dir("design")))
    ...
    # recipe command
    recipe_p = sub.add_parser(
        "recipe",
        help="Export or validate ProcessRecipe JSON/TOML artifacts",
    )
    recipe_sub = recipe_p.add_subparsers(dest="recipe_command")
    recipe_export_p = recipe_sub.add_parser(
        "export-default",
        help="Write the default Protein A affinity-media ProcessRecipe",
    )
    recipe_export_p.add_argument(
        "--output",
        "-o",
        default=str(default_output_dir("recipes", "default_affinity_media_recipe.toml")),
        help="Recipe path to write (.json or .toml)",
    )
    recipe_validate_p = recipe_sub.add_parser(
        "validate",
        help="Load a ProcessRecipe and run resolver-level validation gates",
    )
    recipe_validate_p.add_argument("recipe", help="Recipe JSON/TOML path")
    ...
    batch_p.add_argument("--output", "-o", default=str(default_output_dir("batch")),
    ...
    dossier_p.add_argument("--output", "-o", default=str(default_output_dir("dossier", "dossier.json")),
    ...
    ing_p.add_argument(
        "module", choices=["L1", "M1", "M1QC", "M2", "M3"],
        help=(
            "Validation level to fit. L1 fits DSD references; M1 fits wash "
            "residual retention factors; M1QC fits physical-QC references "
            "for pore, porosity, swelling, and compression/modulus assays; "
            "M2 fits ligand-density, activity-retention, leaching, and wash-fraction assays; "
            "M3 fits binding-capacity, breakthrough-curve, pressure-flow, and affinity entries."
        ),
    )
    ing_p.add_argument(
        "--assay-dir", default=None,
        help="Directory containing AssayRecord JSON files. Defaults depend on module.",
    )
    ing_p.add_argument(
        "--output",
        "-o",
...
def _plain_console(text):
    """Return text that is safe on legacy Windows console encodings."""
    return (
        str(text)
        .replace("\u2265", ">=")
        .replace("\u2264", "<=")
...
def _cmd_run(args):
    ...
    if getattr(args, "gelant", None) is not None:
        ...
        if not args.quiet:
            print(
                ...
                f"(c_Ca_bath = {params.formulation.c_Ca_bath:.1f} mol/m3 "
    ...
    # correct chi, D, and modulus parameters.
    if getattr(args, "cellulose_solvent", None) is not None:
        ...
        if not args.quiet:
            print(
                ...
                f"(N_p={preset.N_p}, chi_PS={preset.chi_PS}, "
...
def _cmd_lifecycle(args):
    """Run the new clean-slate M1 -> M2 -> M3 lifecycle pathway."""
    from .lifecycle.orchestrator import DownstreamProcessOrchestrator
    from .core.recipe_io import (
        load_process_recipe,
    ...
    params = _load_params(args.config)
    if args.recipe:
        recipe = load_process_recipe(Path(args.recipe))
    elif args.config:
        recipe = recipe_from_simulation_parameters(params)
    else:
        recipe = None
    if args.export_recipe:
        from .core.process_recipe import default_affinity_media_recipe

        save_process_recipe(recipe or default_affinity_media_recipe(), Path(args.export_recipe))
    orch = DownstreamProcessOrchestrator(output_dir=Path(args.output))
    result = orch.run(
        recipe=recipe,
        params=params,
        propagate_dsd=not args.no_dsd,
        dsd_mode=args.dsd_mode,
        dsd_max_representatives=args.dsd_max_representatives,
        dsd_run_breakthrough=args.dsd_breakthrough,
        dsd_quantiles=_parse_quantiles_arg(args.dsd_quantiles),
    )

    m1 = result.m1_contract
    fmc = result.functional_media_contract
    m3_method = result.m3_method
    m3 = result.m3_breakthrough

    print()
    print("=== Downstream Lifecycle Results ===")
    print(f"  Weakest evidence tier: {result.weakest_evidence_tier.value}")
    if m1 is not None:
        print(f"  M1 media: bead d50 = {m1.bead_d50 * 1e6:.2f} um, pore = {m1.pore_size_mean * 1e9:.1f} nm")
        if m1.bead_size_distribution is not None:
            dsd = m1.bead_size_distribution
            print(
                "  M1 DSD: "
                f"{len(dsd.diameter_bins_m)} bins; "
                f"d10/d50/d90 = {dsd.d10_m * 1e6:.2f}/"
                f"{dsd.d50_m * 1e6:.2f}/{dsd.d90_m * 1e6:.2f} um"
            )
        print(
            "  M1 wash residuals: "
            f"oil carryover = {m1.residual_oil_volume_fraction:.2%}; "
            f"surfactant = {m1.residual_surfactant_concentration_kg_m3:.3g} kg/m3"
        )
        if m1.washing_model is not None:
            wash = m1.washing_model
            print(
                "  M1 wash model: "
                f"{wash.wash_cycles} x {wash.wash_volume_ratio:g} slurry-volumes; "
                f"oil removal = {wash.oil_removal_efficiency:.1%}"
            )
    if fmc is not None:
        print(f"  M2 ligand: {fmc.installed_ligand or 'none'}; q_max estimate = {fmc.estimated_q_max:.3g} mol/m3")
    if m3 is not None:
    ...
    if m3_method is not None:
        ...
        if m3_method.loaded_elution is not None:
            print(
                "  M3 elution: "
                f"loaded-state recovery = {m3_method.loaded_elution.recovery_fraction:.1%}; "
                f"MB error = {m3_method.loaded_elution.mass_balance_error:.2%}"
            )
        print(
            "  M3 column efficiency: "
            f"N = {m3_method.column_efficiency.theoretical_plates:.0f}; "
            f"HETP = {m3_method.column_efficiency.hetp_m * 1000:.2f} mm; "
            f"asymmetry = {m3_method.column_efficiency.asymmetry_factor:.2f}; "
            f"impurity risk = {m3_method.impurity_clearance.risk}"
        )
    if result.dsd_summary is not None:
        dsd = result.dsd_summary
        print(
            "  DSD screen: "
            f"{dsd.n_quantiles} quantiles; "
            f"mode = {dsd.quantile_selection}; "
            f"q_max mean = {dsd.q_max_weighted_mean_mol_m3:.3g} mol/m3; "
            f"dP range = {dsd.pressure_drop_min_Pa / 1000:.2f}-"
            f"{dsd.pressure_drop_max_Pa / 1000:.2f} kPa; "
            f"dP p95 = {dsd.pressure_drop_weighted_p95_Pa / 1000:.2f} kPa"
        )
        if dsd.breakthrough_simulated:
            print(
                "  DSD breakthrough: "
                f"DBC10 mean/p50/p95 = {dsd.dbc_10_weighted_mean_mol_m3:.3g}/"
                f"{dsd.dbc_10_weighted_p50_mol_m3:.3g}/"
                f"{dsd.dbc_10_weighted_p95_mol_m3:.3g} mol/m3; "
                f"max MB error = {dsd.max_breakthrough_mass_balance_error:.2%}"
            )
    if result.validation.issues:
        print("  Validation issues:")
        for issue in result.validation.issues:
            print(
                f"    [{issue.severity.value}] {issue.module}:{issue.code} - "
                f"{_plain_console(issue.message)}"
            )
    print()


def _cmd_recipe(args):
    """Export or validate ProcessRecipe artifacts."""
    from .core.process_recipe import default_affinity_media_recipe
    from .core.recipe_io import load_process_recipe, save_process_recipe
    from .lifecycle.recipe_resolver import resolve_lifecycle_inputs

    if args.recipe_command == "export-default":
        written = save_process_recipe(default_affinity_media_recipe(), Path(args.output))
        print(f"Default ProcessRecipe written to: {written}")
        return
    if args.recipe_command == "validate":
        recipe = load_process_recipe(Path(args.recipe))
        resolved = resolve_lifecycle_inputs(recipe)
        print("=== ProcessRecipe Validation ===")
        print(f"  steps: {len(recipe.steps)}")
        print(f"  resolved parameters: {len(resolved.resolved_parameters)}")
        if not resolved.validation.issues:
            print("  validation: ok")
            return
        print("  validation issues:")
        for issue in resolved.validation.issues:
            print(
                f"    [{issue.severity.value}] {issue.module}:{issue.code} - "
                f"{_plain_console(issue.message)}"
            )
        if resolved.validation.blockers:
            sys.exit(2)
        return
    raise SystemExit("Missing recipe subcommand. Use `dpsim recipe --help`.")
...
def _cmd_ui(args):
    ...
    print(f"Launching dpsim UI on port {args.port}...")
...
def _cmd_ingest(args):
    ...
    default_assay_dirs = {
        "L1": Path("data/validation/l1_dsd/assays"),
        "M1": Path("data/validation/m1_washing/assays"),
        "M1QC": Path("data/validation/m1_physical_qc/assays"),
        "M2": Path("data/validation/m2_capacity/assays"),
        "M3": Path("data/validation/m3_binding/assays"),
    }
    default_outputs = {
        "L1": default_output_dir("calibration", "l1_dsd_fit.json"),
        "M1": default_output_dir("calibration", "m1_washing_fit.json"),
        "M1QC": default_output_dir("calibration", "m1_physical_qc_fit.json"),
        "M2": default_output_dir("calibration", "m2_functionalization_fit.json"),
        "M3": default_output_dir("calibration", "m3_binding_fit.json"),
    }

    assay_dir = Path(args.assay_dir) if args.assay_dir else default_assay_dirs[args.module]
    ...
    if args.module == "L1":
        ...
        fitter_name = "stub_mean (v7.0)"
        empty_hint = "no DROPLET_SIZE_DISTRIBUTION assays?"
    elif args.module == "M1":
        entries = fit_m1_washing_to_calibration_entries(records)
        fitter_name = "inverse_well_mixed_extraction"
        empty_hint = "no RESIDUAL_OIL or RESIDUAL_SURFACTANT assays?"
    elif args.module == "M1QC":
        entries = fit_m1_physical_qc_to_calibration_entries(records)
        fitter_name = "assay_reference_mean"
        empty_hint = (
            "no PORE_SIZE, POROSITY, SWELLING_RATIO, BULK_MODULUS, "
            "or COMPRESSION_MODULUS assays?"
        )
    elif args.module == "M2":
        entries = fit_m2_functionalization_to_calibration_entries(records)
        fitter_name = "m2_functionalization_reference"
        empty_hint = (
            "no LIGAND_DENSITY, ACTIVITY_RETENTION, LIGAND_LEACHING, "
            "or FREE_PROTEIN_WASH_FRACTION assays?"
        )
    elif args.module == "M3":
        entries = fit_m3_binding_to_calibration_entries(records)
        fitter_name = "m3_binding_capacity_breakthrough_pressure_wls"
        empty_hint = (
            "no STATIC_BINDING_CAPACITY, DYNAMIC_BINDING_CAPACITY, "
            "or PRESSURE_FLOW_CURVE assays?"
        )
    else:
        print(f"Module {args.module} ingestion is not wired.", file=sys.stderr)
        sys.exit(2)
    if not entries:
        print(f"WARNING: fitter produced 0 CalibrationEntries from "
              f"{len(records)} records ({empty_hint}).")
        return

    out_path = Path(args.output) if args.output else default_outputs[args.module]
    fit_metadata = {
        "module": args.module,
