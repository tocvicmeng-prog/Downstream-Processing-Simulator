...
from ..level1_emulsification.washing import solve_m1_washing
...
class PipelineOrchestrator:
    ...
    def __init__(self, db: Optional[PropertyDatabase] = None,
        ...
        self.output_dir = Path(output_dir) if output_dir else default_output_dir("m1")
    ...
    def run_single(self, params: SimulationParameters,
        ...
        # M1 process calibrations target formulation/process fields such as
        # wash retention factors. Apply them before property interpolation and
        # before export_for_module2() computes wash residuals. Use a copied
        # formulation so caller-owned SimulationParameters are not mutated.
        if run_context is not None and run_context.calibration_store is not None:
            store = run_context.calibration_store
            if store.has_calibration_for("M1"):
                params = copy.copy(params)
                params.formulation = copy.copy(params.formulation)
                params.formulation, overrides_log = store.apply_to_model_params(
                    "M1", params.formulation,
                )
...
def export_for_module2(result: FullResult,
    ...
    # M1 full bead distribution and wash residual state. These are exported
    # explicitly so M2/M3 do not silently collapse fabrication to a single d50.
    if params.emulsification.mode == "stirred_vessel":
        physical_phi_d = params.formulation.phi_d_from_volumes
    else:
        physical_phi_d = params.formulation.phi_d
    bead_size_distribution = BeadSizeDistributionPayload.from_emulsification_result(
        emul,
        total_volume_fraction=physical_phi_d,
    )
    washing_result = solve_m1_washing(params)
    washing_assumptions = (
        list(washing_result.assumptions)
        + list(washing_result.warnings)
        + ["Replace M1 washing estimates with residual oil/surfactant assays before process release."]
    )
    ...
    if emul.span > 1.0:
        notes.append("Broad bead DSD; propagate quantiles into M2/M3 rather than using d50 only")
    notes.append("M1 wash residuals use an uncalibrated well-mixed extraction model until assays are supplied")
    ...
    contract = M1ExportContract(
        ...
        bead_size_distribution=bead_size_distribution,
        oil_removal_efficiency=washing_result.oil_removal_efficiency,
        residual_oil_volume_fraction=washing_result.residual_oil_volume_fraction,
        residual_surfactant_concentration_kg_m3=(
            washing_result.residual_surfactant_concentration_kg_m3
        ),
        washing_assumptions=washing_assumptions,
        washing_model=washing_result,
    )
