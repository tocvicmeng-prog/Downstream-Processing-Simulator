"""Unified Monte Carlo uncertainty quantification for dpsim.
...
# KernelConfig (lives at params.emulsification.kernels); M1 posteriors target
# FormulationParameters; later modules target MaterialProperties.
_KERNEL_MODULES = {"L1"}
_FORMULATION_MODULES = {"M1"}
...
def _apply_posterior_samples(
    ...
    L1 -> KernelConfig at ``params.emulsification.kernels``; M1 ->
    FormulationParameters; L2-L4/M2-M3 -> MaterialProperties. Malformed
    names, unknown modules, and unknown attributes are logged at DEBUG and
    skipped.
    ...
    for name, value in posterior_samples.items():
        ...
        if module in _KERNEL_MODULES:
            ...
            target = kernels
        elif module in _FORMULATION_MODULES:
            target = params.formulation
...
def _mc_one_sample(item, crosslinker_key, uv_intensity, l2_mode):
    ...
    Returns (d32, pore, G_DN, residual_oil, residual_surfactant) on success,
    None on failure.
    ...
    try:
        ...
        washing = solve_m1_washing(params_i)
        return (
            emul.d32,
            gel.pore_size_mean,
            mech.G_DN,
            washing.residual_oil_volume_fraction,
            washing.residual_surfactant_concentration_kg_m3,
        )
...
class UnifiedUncertaintyEngine:
    """Single-entrypoint MC engine for dpsim uncertainty quantification.
    ...
    def run_m1l4(
        ...
        residual_oil_samples: list[float] = []
        residual_surfactant_samples: list[float] = []
        n_failed = 0
        for out in outputs:
            if out is None:
                n_failed += 1
                continue
            d32_i, pore_i, G_DN_i, residual_oil_i, residual_surfactant_i = out
            d32_samples.append(d32_i)
            pore_samples.append(pore_i)
            G_DN_samples.append(G_DN_i)
            residual_oil_samples.append(residual_oil_i)
            residual_surfactant_samples.append(residual_surfactant_i)
        ...
        outputs_list = [
            ...
            _build_output(
                "residual_oil_volume_fraction",
                "fraction",
                residual_oil_samples,
                n_failed,
            ),
            _build_output(
                "residual_surfactant_concentration",
                "kg/m3",
                residual_surfactant_samples,
                n_failed,
            ),
        ]
