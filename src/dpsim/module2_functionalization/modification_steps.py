...
def solve_modification_step(
    ...
    if step.step_type != ModificationStepType.WASHING and reagent_profile.target_acs != step.target_acs:
        raise ValueError(
    ...
    # Washing is a non-reactive cleanup operation. It should still be allowed
    # after quenching or after all activated sites are consumed because it
    # represents diffusion-out of residual soluble reagent, not further surface
    # chemistry on a remaining ACS pool.
    if step.step_type == ModificationStepType.WASHING:
        result = _solve_washing_step(
            step, acs_state, surface_model, surface_model.bead_volume,
        )
...
def _build_step_manifest(
    ...
    diagnostics: dict = {
        ...
        "pH": float(step.ph),
        "temperature_K": float(step.temperature),
        "reagent_concentration_mol_m3": float(step.reagent_concentration),
    }
    if result.reaction_diagnostics:
        diagnostics.update(result.reaction_diagnostics)
    if conservation_violations:
    ...
    denaturation_factor = diagnostics.get("denaturation_factor")
    if isinstance(denaturation_factor, (int, float)) and denaturation_factor < 1.0:
        assumptions.append(
            "Protein denaturation factor estimated from pH/temperature exposure; "
            "requires wet-lab activity assay calibration."
        )

    valid_domain: dict = {}
    # Reagent profiles carry pH/T validity windows. Surface them directly on
    # the manifest so lifecycle-level domain gates can detect extrapolation
    # without re-reading the reagent library.
    valid_domain["pH"] = (
        float(getattr(reagent_profile, "ph_min", 0.0)),
        float(getattr(reagent_profile, "ph_max", 14.0)),
    )
    valid_domain["temperature_K"] = (
        float(getattr(reagent_profile, "temperature_min", 273.15)),
        float(getattr(reagent_profile, "temperature_max", 373.15)),
    )
...
def _solve_ligand_coupling_step(
    ...
    cr.activity_retention_factor = _effective_activity
    cr.denaturation_factor = 1.0
    ...
    product_type = step.product_acs or reagent_profile.product_acs
    if product_type is not None and cr.sites_coupled > 0.0:
        _add_ligand_product_profile(
            acs_state,
            source_profile=target_profile,
            product_type=product_type,
            sites_created=cr.sites_coupled,
        )
    ...
    return ModificationResult(
        ...
        reaction_diagnostics={
            "sites_coupled_mol_per_particle": float(cr.sites_coupled),
            "sites_hydrolyzed_mol_per_particle": float(cr.sites_hydrolyzed),
            "hydrolysis_fraction": float(cr.hydrolysis_fraction),
            "reagent_remaining_fraction": float(cr.reagent_remaining_fraction),
            "site_balance_error": float(cr.site_balance_error),
            "activity_retention_factor": float(cr.activity_retention_factor),
            "denaturation_factor": float(cr.denaturation_factor),
            "effective_activity_retention": float(_effective_activity),
            "target_site_type": step.target_acs.value,
            "product_site_type": product_type.value if product_type else "",
        },
    )
...
def _protein_denaturation_factor(
    step: ModificationStep,
    reagent_profile: ReagentProfile,
) -> float:
    """Estimate protein activity loss from out-of-domain pH/T exposure.

    This is a conservative screening model, not a stability claim. Inside the
    reagent profile's pH and temperature domain the factor is 1.0. Outside that
    domain it decays exponentially with exposure time, giving recipe validation
    and assay contracts a quantitative placeholder until ligand-specific wet-lab
    activity retention data are ingested.
    """

    if not getattr(reagent_profile, "is_macromolecule", False):
        return 1.0
    hours = max(float(step.time), 0.0) / 3600.0
    if hours <= 0.0:
        return 1.0
    temp_low = float(getattr(reagent_profile, "temperature_min", 273.15))
    temp_high = float(getattr(reagent_profile, "temperature_max", 373.15))
    ph_low = float(getattr(reagent_profile, "ph_min", 0.0))
    ph_high = float(getattr(reagent_profile, "ph_max", 14.0))
    temp_excursion = max(temp_low - step.temperature, step.temperature - temp_high, 0.0)
    ph_excursion = max(ph_low - step.ph, step.ph - ph_high, 0.0)
    if temp_excursion <= 0.0 and ph_excursion <= 0.0:
        return 1.0
    exponent = -(0.03 * temp_excursion + 0.10 * ph_excursion) * hours
    return max(0.0, min(math.exp(exponent), 1.0))


def _add_ligand_product_profile(
    acs_state: dict[ACSSiteType, ACSProfile],
    *,
    source_profile: ACSProfile,
    product_type: ACSSiteType,
    sites_created: float,
) -> None:
    """Create a downstream ACS inventory for coupled ligand functionality.

    The source ACS profile still owns conservation for the covalent coupling
    event. This product profile represents the installed ligand's own
    downstream-reactive inventory, for example NTA/IDA chelators that can be
    metal charged after epoxide coupling.
    """

    if sites_created <= 0.0:
        return
    if product_type in acs_state:
    ...
    acs_state[product_type] = ACSProfile(
        ...
        total_density=0.0,
        accessible_density=0.0,
        accessibility_model=source_profile.accessibility_model,
        uncertainty_fraction=source_profile.uncertainty_fraction,
    )
...
def _solve_protein_coupling_step(
    ...
    if target_profile.site_type == ACSSiteType.MALEIMIDE:
        ...
        if _avail_before > 0 and step.time > 0:
            ...
            maleimide_decayed_sites = _sites_decayed
            logger.info(
    ...
    retained_activity = min(activity_ret * spacer_mult, 1.0)  # cap at 1.0
    denaturation_factor = _protein_denaturation_factor(step, reagent_profile)
    effective_activity = retained_activity * denaturation_factor
    cr.activity_retention_factor = retained_activity
    cr.denaturation_factor = denaturation_factor
    target_profile.ligand_coupled_sites += cr.sites_coupled
    ...
    notes = (
        ...
        f"coupled={cr.sites_coupled:.4e}, functional={cr.sites_coupled * effective_activity:.4e} "
        f"(activity_retention={retained_activity:.2f}, denaturation={denaturation_factor:.2f}). "
        "RANKING_ONLY."
    )
    if cr.warnings:
        notes += f" Warnings: {'; '.join(cr.warnings)}"
    ...
    return ModificationResult(
        ...
        reaction_diagnostics={
            "sites_coupled_mol_per_particle": float(cr.sites_coupled),
            "sites_hydrolyzed_mol_per_particle": float(maleimide_decayed_sites),
            "hydrolysis_fraction": float(cr.hydrolysis_fraction),
            "reagent_remaining_fraction": float(cr.reagent_remaining_fraction),
            "site_balance_error": float(cr.site_balance_error),
            "steric_accessibility_fraction": float(cr.steric_accessibility_fraction),
            "activity_retention_factor": float(cr.activity_retention_factor),
            "denaturation_factor": float(cr.denaturation_factor),
            "effective_activity_retention": float(effective_activity),
            "target_site_type": step.target_acs.value,
        },
    )
...
def _solve_quenching_step(
    ...
    return ModificationResult(
        ...
        conversion=cr.conversion, delta_G_DN=0.0, notes=notes,
        reaction_diagnostics={
            "sites_blocked_mol_per_particle": float(cr.sites_blocked),
            "reagent_remaining_fraction": float(cr.reagent_remaining_fraction),
            "site_balance_error": float(cr.site_balance_error),
            "target_site_type": step.target_acs.value,
        },
...
def _solve_spacer_arm_step(
    ...
    return ModificationResult(
        ...
        reaction_diagnostics={
            "sites_consumed_mol_per_particle": float(sites_consumed),
            "sites_created_mol_per_particle": float(sites_created),
            "sites_bridged_mol_per_particle": float(sites_bridged),
            "reagent_remaining_fraction": float(reagent_remaining),
            "distal_group_yield": float(distal_yield),
            "target_site_type": step.target_acs.value,
            "product_site_type": product_type.value,
        },
    )
