...
class CouplingResult:
    ...
    steric_accessibility_fraction: float = 1.0  # [0,1] remaining RSA-like access after coupling
    activity_retention_factor: float = 1.0  # [0,1] coupled ligand activity retained
    denaturation_factor: float = 1.0   # [0,1] protein activity retained after pH/T exposure
...
def solve_steric_coupling(
    ...
    steric_fraction = 1.0
    if max_coupled_mol_per_particle > 0.0:
        steric_fraction = 1.0 - sites_coupled / max_coupled_mol_per_particle
    steric_fraction = max(0.0, min(float(steric_fraction), 1.0))
    ...
    return CouplingResult(
        ...
        steric_accessibility_fraction=steric_fraction,
        solver_success=sol.success,
        solver_message=sol.message if not sol.success else "",
        site_balance_error=balance_error,
        warnings=warnings,
