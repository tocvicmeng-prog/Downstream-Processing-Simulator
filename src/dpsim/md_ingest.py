...
records into dpsim MaterialProperties.

dpsim itself does **not** run MARTINI simulations — we accept
...
- ``beads``             — dict mapping dpsim role → MARTINI bead
                          type, e.g. ``{"polymer": "P3", ...}``
...
class MartiniRecord:
    ...
    beads : dict
        Mapping dpsim role → MARTINI bead type.
    chi : dict
