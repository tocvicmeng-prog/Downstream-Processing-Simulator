...
from scipy.sparse.linalg import bicgstab, factorized, spsolve, splu
...
class CahnHilliard2DSolver:
    ...
    def solve(self, params: SimulationParameters, props: MaterialProperties,
        ...
        # Reported domain is the droplet cross-section represented by this
        # local 2D patch. The numerical RVE is capped separately to avoid
        # ill-conditioned 32^2 SuperLU/iterative solves on Windows SciPy while
        # preserving a deterministic morphology screen.
        L_domain_report = min(2.0 * R_droplet, max(5.0 * lambda_CH, 0.5e-6))
        L_domain = min(L_domain_report, 3.0e-6)
        if L_domain_report < 2.0 * R_droplet:
            logger.info("L2: domain %.1f nm < droplet %.1f nm (local-patch RVE mode)",
                        L_domain_report * 1e9, 2 * R_droplet * 1e9)
        ...
        while t < t_final and step < max_steps:
            ...
            if need_rebuild:
                ...
                LM_L = L_M_csc @ L_std_csc
                A_mat = I_mat + dt * C_contract * L_M_csc + dt * kappa * LM_L
                # Repeated SuperLU factorization can stall on some Windows
                # SciPy builds for the 32^2 smoke grid once mobility and dt
                # drift. Keep direct LU only for tiny systems; use the
                # iterative path below for normal 2D smoke grids.
                if total <= 256:
                    try:
                        A_factor = splu(A_mat.tocsc())
                    except Exception:
            ...
            # Solve using cached LU factorization or iterative sparse solve.
            try:
