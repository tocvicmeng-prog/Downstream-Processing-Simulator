...
def _test_output_dir(name: str) -> Path:
    """Return a DPSim-owned test output path instead of root-level /tmp."""
    path = default_output_dir("tests", name)
...
class TestEngineTargetSpec:
    def test_default_uses_legacy_3d(self):
        engine = OptimizationEngine(
            ...
            output_dir=_test_output_dir("_dpsim_f3b_test_legacy"),
        )
    ...
    def test_target_spec_sizes_arrays(self):
        ...
        engine = OptimizationEngine(
            n_initial=2, max_iterations=1,
            output_dir=_test_output_dir("_dpsim_f3b_test_spec"),
            target_spec=spec,
        )
    ...
    def test_invalid_target_spec_rejected_at_ctor(self):
        ...
        with pytest.raises(ValueError, match="no active dimension"):
            OptimizationEngine(
                n_initial=2, max_iterations=1,
                output_dir=_test_output_dir("_dpsim_f3b_test_empty"),
    ...
    def test_robust_requires_target_spec(self):
        ...
        with pytest.raises(ValueError, match="requires target_spec"):
            OptimizationEngine(
                n_initial=2, max_iterations=1,
                output_dir=_test_output_dir("_dpsim_f4_no_target"),
    ...
    def test_robust_needs_enough_samples(self):
        ...
        with pytest.raises(ValueError, match="robust_n_samples"):
            OptimizationEngine(
                n_initial=2, max_iterations=1,
                output_dir=_test_output_dir("_dpsim_f4_few_samples"),
                target_spec=spec,
                robust_variance_weight=0.5,
                robust_n_samples=1,
            )
    ...
    def test_robust_happy_path_constructs(self):
        ...
        engine = OptimizationEngine(
            n_initial=2, max_iterations=1,
            output_dir=_test_output_dir("_dpsim_f4_ok"),
            target_spec=spec,
...
class TestDesignCLIParser:
    def test_parser_accepts_design_subcommand(self):
        ...
        from dpsim.__main__ import main
        with patch("sys.argv", ["dpsim", "design", "--help"]):
    ...
    def test_empty_target_spec_rejected_by_cli(self):
        """`dpsim design` with NO target flags must raise
        SystemExit with a clear error (TargetSpec.validate fails)."""
        from dpsim.__main__ import main
        with patch("sys.argv", ["dpsim", "design"]):
            with pytest.raises(SystemExit) as exc:
                main()
    ...
    def test_design_dispatches_to_engine_with_target(self):
        ...
        with patch(
            "dpsim.optimization.engine.OptimizationEngine.run",
            return_value=fake_state,
        ) as mock_run, patch(
            "dpsim.optimization.analysis.pareto_summary",
            return_value="ok",
        ), patch(
            "dpsim.optimization.analysis.best_compromise",
            return_value=0,
        ), patch("sys.argv", [
            "dpsim", "design",
