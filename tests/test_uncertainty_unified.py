...
class TestUnifiedEngine:
    ...
    def test_m1_wash_posterior_reaches_residual_outputs(self, smoke_params):
        """P2: M1 wash calibration posteriors perturb contamination-risk outputs."""
        from dpsim.calibration.calibration_data import CalibrationEntry
        from dpsim.calibration.calibration_store import CalibrationStore
        ...
        store.add(CalibrationEntry(
            ...
            units="1",
            confidence="medium",
            source_reference="synthetic residual oil fit",
            target_module="M1",
        ...
        engine = UnifiedUncertaintyEngine(calibration_store=store)
        result = engine.run_m1l4(smoke_params, n_samples=4, seed=8)

        assert UncertaintyKind.CALIBRATION_POSTERIOR in result.kinds_sampled
        residual_oil = result.get("residual_oil_volume_fraction")
        residual_surfactant = result.get("residual_surfactant_concentration")
        assert residual_oil is not None
        assert residual_surfactant is not None
        assert residual_oil.units == "fraction"
        assert residual_oil.raw_samples is not None
        assert residual_oil.raw_samples.shape[0] >= 1
    ...
    def test_unknown_posterior_attribute_skipped(self, smoke_params, caplog):
        ...
        caplog.set_level(logging.DEBUG, logger="dpsim.uncertainty_unified")
        store = CalibrationStore()
        store.add(CalibrationEntry(
    ...
    def test_legacy_modules_are_gone(self):
        ...
        with pytest.raises(ImportError):
            import dpsim.uncertainty_core  # noqa: F401
        with pytest.raises(ImportError):
            import dpsim.uncertainty_propagation  # noqa: F401
